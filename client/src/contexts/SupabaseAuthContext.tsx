...
export function SupabaseAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshDashboard = useCallback(async () => {
    ...
    const [profileResult, membershipResult, paymentsResult] = await Promise.all([
      ...
      supabase.from("payments").select("id, amount, currency, payment_method, status, submitted_date, created_at").eq("user_id", currentUser.id).order("created_at", { ascending: false }),
    ]);
  ...
  const value = useMemo<SupabaseAuthContextValue>(() => ({
    ...
    membershipLabel: formatMembershipStatus(membership),
    signIn,
    signUp,
    signOut,
    refreshDashboard,
  }), [dashboardLoading, loading, membership, payments, profile, refreshDashboard, session, signIn, signOut, signUp]);
