...
type SupabaseAuthContextValue = { session: Session | null; user: User | null; profile: Profile | null; membership: MembershipRecord | null; payments: PaymentRecord[]; loading: boolean; dashboardLoading: boolean; isAuthenticated: boolean; membershipLabel: string; signIn: (email: string, password: string) => Promise<AuthResult>; signUp: (fullName: string, email: string, password: string) => Promise<AuthResult>; signOut: () => Promise<void>; refreshDashboard: () => Promise<void> };
const SupabaseAuthContext = createContext<SupabaseAuthContextValue | null>(null);

export function SupabaseAuthProvider({ children }: { children: React.ReactNode }) {
  const [session, setSession] = useState<Session | null>(null); const [loading, setLoading] = useState(true); const [dashboardLoading, setDashboardLoading] = useState(false); const [profile, setProfile] = useState<Profile | null>(null); const [membership, setMembership] = useState<MembershipRecord | null>(null); const [payments, setPayments] = useState<PaymentRecord[]>([]);
  const clearDashboard = useCallback(() => { setProfile(null); setMembership(null); setPayments([]); }, []);
  const refreshDashboard = useCallback(async () => { const currentUser = session?.user; if (!currentUser) { clearDashboard(); return; } setDashboardLoading(true); const [profileResult, membershipResult, paymentsResult] = await Promise.all([supabase.from("profiles").select("id, full_name, phone, role, created_at").eq("id", currentUser.id).maybeSingle(), supabase.from("memberships").select("id, status, starts_at, expires_at").eq("user_id", currentUser.id).maybeSingle(), supabase.from("payments").select("id, amount, currency, payment_method, status, submitted_date, created_at, payment_proof_key, payment_proof_filename, payment_proof_content_type").eq("user_id", currentUser.id).order("created_at", { ascending: false })]); setProfile((profileResult.data as Profile | null) ?? null); setMembership((membershipResult.data as MembershipRecord | null) ?? null); setPayments((paymentsResult.data as PaymentRecord[] | null) ?? []); setDashboardLoading(false); }, [clearDashboard, session?.user]);
  useEffect(() => { let mounted = true; supabase.auth.getSession().then(({ data: { session: nextSession } }) => { if (!mounted) return; setSession(nextSession); setLoading(false); }); const { data: listener } = supabase.auth.onAuthStateChange((_event, nextSession) => { setSession(nextSession); setLoading(false); if (!nextSession) clearDashboard(); }); return () => { mounted = false; listener.subscription.unsubscribe(); }; }, [clearDashboard]);
