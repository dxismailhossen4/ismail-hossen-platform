...
export default function PaymentVerificationDashboard() {
  ...
  const proofQuery = trpc.paymentProof.getSignedUrl.useQuery(
    { accessToken: session?.access_token ?? "", paymentId: proofPaymentId ?? EMPTY_ID },
    { enabled: Boolean(session?.access_token && proofPaymentId), retry: false },
  );
